...
final class QwenElementwise {
    ...
    init(context: MetalContext) throws {
        ...
        self.prefillDeltaParametersFloatPSO = try context.pipeline(
            "qwen_prefill_delta_parameters_float")
        self.prefillGatedNormFloatPSO = try context.pipeline(
            "qwen_prefill_gated_rmsnorm_float")
    ...
    func encodeDeltaParametersBatchFloat(commandBuffer: MTLCommandBuffer,
                                         a: MTLBuffer,
                                         betaInput: MTLBuffer,
                                         aLog: MTLBuffer,
                                         aLogOffset: Int = 0,
                                         ...
                                         headCount: UInt32) {
        guard let encoder = commandBuffer.makeComputeCommandEncoder() else { return }
        encoder.setComputePipelineState(prefillDeltaParametersFloatPSO)
        encoder.setBuffer(a, offset: 0, index: 0)
        encoder.setBuffer(betaInput, offset: 0, index: 1)
        encoder.setBuffer(aLog, offset: aLogOffset, index: 2)
        encoder.setBuffer(dtBias, offset: dtBiasOffset, index: 3)
    ...
    func encodeGatedNormBatchFloat(commandBuffer: MTLCommandBuffer,
                                   input: MTLBuffer,
                                   gate: MTLBuffer,
                                   weight: MTLBuffer,
                                   weightOffset: Int = 0,
                                   ...
                                   epsilon: Float) {
        guard let encoder = commandBuffer.makeComputeCommandEncoder() else { return }
        encoder.setComputePipelineState(prefillGatedNormFloatPSO)
        encoder.setBuffer(input, offset: 0, index: 0)
        encoder.setBuffer(gate, offset: 0, index: 1)
        encoder.setBuffer(weight, offset: weightOffset, index: 2)
        encoder.setBuffer(output, offset: 0, index: 3)
